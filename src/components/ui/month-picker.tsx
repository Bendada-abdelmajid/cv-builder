...
//         currentMonth ?  format(currentMonth, 'yyyy') :
...
export default function MonthPicker({
    ...
    const currentMonthDate = currentMonth ? parse(currentMonth, 'MMMM yyyy', new Date()) : undefined;

    const [currentYear, setCurrentYear] = React.useState(
        currentMonthDate ? format(currentMonthDate, 'yyyy') : format(new Date(), 'yyyy')
    );
    const firstDayCurrentYear = parse(currentYear, 'yyyy', new Date());
                    ...
                    {months.map((month) => (
                        ...
                        >
                            <button
                                ...
                                className={cn(
                                    'inline-flex h-9 w-12 items-center justify-center rounded-md p-0 text-sm font-normal ring-offset-white transition-colors hover:bg-slate-100 hover:text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 aria-selected:opacity-100 dark:ring-offset-slate-950 dark:hover:bg-slate-800 dark:hover:text-slate-50 dark:focus-visible:ring-slate-800',
                                    isEqual(month, currentMonthDate || new Date()) &&
                                    'bg-slate-900 text-slate-50 hover:bg-slate-900 hover:text-slate-50 focus:bg-slate-900 focus:text-slate-50 dark:bg-slate-50 dark:text-slate-900 dark:hover:bg-slate-50 dark:hover:text-slate-900 dark:focus:bg-slate-50 dark:focus:text-slate-900',
                                    !isEqual(month, currentMonthDate || new Date()) &&
                                    isEqual(month, getStartOfCurrentMonth()) &&
                                    'bg-slate-100 text-slate-900 dark:bg-slate-800 dark:text-slate-50'
                                ...
                                tabIndex={-1}
                                type="button"
                                onClick={() => onMonthChange(format(month, 'MMMM yyyy'))}
                            >
                                <time dateTime={format(month, 'yyyy-MM-dd')}>
                                    {format(month, 'MMM')}
                                </time>
